...
pub(crate) async fn index<'a>(
    ...
    page(&resource.data_view, None, index::content())
}
...
pub(crate) async fn object<'a>(
    ...
    let object = resource.data_view.index(&key).ok_or_else(|| {
        let id = (&id).to_owned();
        Error::ObjectNotFound { group, id }
    })?;
    Ok(page(
        &resource.data_view,
        Some(&key),
...
fn page(data: &DataView, active_key: Option<&Key>, content: Markup) -> Result<Html<String>, Error> {
    let page = html! {
        (DOCTYPE)
        html {
